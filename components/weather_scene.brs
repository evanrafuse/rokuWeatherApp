function init()
  ...
  loadJSON("pkg:/images/cities.json")
  ' m.category_screen.observeField("category_selected", "onCategorySelected")
  ' m.category_screen.setFocus(true)
end function

function onKeyEvent(key, press) as Boolean
    ? "[weather_scene] onKeyEvent", key, press
  return false
end function
...
sub onFeedResponse(obj)
  ...
  if data <> Invalid and data.name <> invalid
...
sub loadJSON(url)
  jsonAsString = ReadAsciiFile(url)
  data = parseJSON(jsonAsString)
  ? "Data from response: "; data
  if data <> Invalid and data.regions <> invalid
    ' m.category_screen.visible = false
    m.content_screen.visible = true
    m.content_screen.feed_data = data
  else
